...
const deployStakingVault: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  try {
    ...
    // Check if StakingVault is already deployed
    ...
    try {
      ...
      console.log(`\n✅ Found existing StakingVault at: ${stakingVaultDeployment.address}`);
    } catch {
      // If not found, ask for confirmation to deploy
      isNewDeployment = true;
      const deployConfirmed = await promptForConfirmation(
        `Deploy new StakingVault contract with RewardToken at ${rewardTokenDeployment.address}?`,
      );
      ...
      stakingVaultDeployment = await deploy("StakingVault", {
        from: deployer,
    ...
    if (network !== "localhost" && network !== "hardhat") {
      ...
      if (isNewDeployment) {
        ...
        try {
          ...
          console.error("❌ Failed to verify StakingVault");
        }
      }
    }
  } catch (error) {
