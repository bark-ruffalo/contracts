...
// Configuration
const TOTAL_AMOUNT = ethers.parseUnits("9.3", 18); // Total ETH amount to distribute
...
// Calculate ETH amounts based on per mill values
...
async function main() {
  ...
  if (SHOULD_EXECUTE) {
    ...
    distribution.sort((a, b) => (a.ethAmount < b.ethAmount ? -1 : 1));

    // Check ETH balance
    const balance = await provider.getBalance(signer.address);
    if (balance < TOTAL_AMOUNT) {
      throw new Error(
        `Insufficient ETH balance. Have: ${ethers.formatUnits(balance, 18)}, Need: ${ethers.formatUnits(TOTAL_AMOUNT, 18)}`,
      );
    }

    // Get gas price and estimate gas for ETH transfers
    const feeData = await provider.getFeeData();
    const gasPrice = feeData.gasPrice;

    console.log("\n🚀 Starting ETH distribution...");
    ...
    try {
      ...
      for (let i = 0; i < distribution.length; i++) {
        ...
        console.log(`Amount: ${ethers.formatUnits(staker.ethAmount, 18)} ETH`);
        ...
        const tx = await signer.sendTransaction({
          to: staker.address,
          value: staker.ethAmount,
          gasLimit: 41000, // Standard gas limit for ETH transfers
          gasPrice,
        });
